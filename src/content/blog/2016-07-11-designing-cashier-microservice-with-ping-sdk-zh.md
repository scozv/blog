---
title: "基于Ping++的收银微服务架构设计"
description: '使用Ping++可以快速接入不同的支付渠道。 通常情况下，每一个应用需要在Ping++后台系统中，对应地创建一个应用， 用来调用Ping++的接口。 本文尝试设计这样的解决方案——该方案仅有一个Ping++应用，但同时满足多个（独立）应用的支付场景。 本文提到的解决方案，目前处在设计阶段。 本文将提到如下话题： 使用Ping++完成一般的支付流程 来自百货大楼购物的启发 设计可供集中收银的微...'
category: pattern
tags:
  - ping++
  - microservice
